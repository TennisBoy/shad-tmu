...
> Self-contained project. It does **not** depend on anything outside this `shad-tmu/` folder, so it
> can be lifted into its own git repo at any time. If/when that happens, this file becomes the
> repo's root `CLAUDE.md`. Do not add imports from the parent wiki.
...
- Own Azure Static Web App + own workflow `.github/workflows/deploy-shad.yml` (in the parent repo
  until this folder is split out). Trigger: `shad-tmu/**`. Static, no build step.
- Needs repo secret `AZURE_STATIC_WEB_APPS_API_TOKEN_SHAD`.
- Google Analytics: set `ga_id` in `data.js`. Empty string disables it cleanly.
